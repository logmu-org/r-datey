...
**datey** provides three atomic[^atomic] S3 classes:
...
[^atomic]: An atomic type can be stored in a single R vector without any additional special handling. Although they have properties such as `t$year` for a `datey`
or `interval$start` for a `datey_interval`, these properties are generated on the fly --
they are not accessing separate underlying data vectors.
