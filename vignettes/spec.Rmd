...
1. Real-world date arithmetic[^Undefined], including calculating dates for output.
...
1. Otherwise the resulting number of clicks is the *duration in years* multiplied by 534&#x202F;360 and then rounded to `integer` using banker's rounding.
...
It is *not* required or expected that the above operations will be checked for `integer` overflow -- they can reasonably be supposed to be safe within the domain of operation (i.e. date inputs that have already been checked for validity) and datey exists to facilitate performant calculations.
