...
The following are the available binary operations on `datey`,`durationy`
and `datey_interval` only operands, and their meaning:
...
| `datey` | `%to%` | `datey` | `datey_interval` | Create a date interval – syntactic sugar for [`datey_interval()`](https://r-datey.logmu.org/reference/datey_interval.md) |
| `datey_interval` | `%includes%` | `datey` | `logical` | Whether an interval includes a date – syntactic sugar for [`interval_includes()`](https://r-datey.logmu.org/reference/interval_includes.md) |
...
`datey`s and `durationy`s can also be mixed with numeric operands, in
which case the `datey` or `durationy` is first converted to years, The
following operations are implemented

- Comparison, i.e. a `datey` or `durationy` `==` `!=` `<` `<=` `>` `>=`
  numeric or vice versa. Result is `logical`.

- `datey` addition and subtraction, i.e. a `datey` `+` `-` a numeric or
  vice versa. Result is `double`.

- `durationy` arithmetic, i.e. a `durationy` `+` `-` `*` `/` a numeric
  or vice versa. Result is `double`.

- The `%to%` operator accepts numbers, which are treated as years and
  coerced to `datey`.

- The `%includes%` operator accepts a number as its right hand operand,
  which is treated as years and coerced to `datey`.

When applied to `datey_interval`s, `&` is the 'intersection' operator.
For intervals that do not intersect the result of `&` depends on whether
the intervals are adjacent. If they are adjacent then the result is an
empty interval starting (and ending) at the point in time they touch.
Otherwise it is `NA_datey_interval_`. You can test whether intervals `a`
and `b` intersect using `is_collapsed(a & b)`.
